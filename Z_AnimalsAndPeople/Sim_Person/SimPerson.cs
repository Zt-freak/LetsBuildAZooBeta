  ...
  internal class SimPerson
  {
      ...
      WalkingPerson walkingperson)
    {
      ...
      if (IsEmployee)
      {
        ...
        {
          if (persontype != AnimalType.MaleZookeeper && persontype != AnimalType.FemaleZookeeper && (persontype != AnimalType.MaleAsianZookeeper && persontype != AnimalType.FemaleAsianZookeeper) && (persontype != AnimalType.MaleDarkZookeeper && persontype != AnimalType.FemaleDarkZookeeper))
      ...
      {
        if (persontype == AnimalType.TigerKing || persontype == AnimalType.BlackMarketDealer)
        {
          this.roleinsociety = RoleInSociety.BlackMarket;
          this.customertype = CustomerType.BlackMarket;
          this.memberofthepublic = new MemberOfThePublic(persontype, cellblockcontainer, true, player, this.customertype);
        ...
        else if (EnemyData.IsThisAAnimalWelfarePerson(persontype))
        {
        ...
        else if (EnemyData.IsThisACriticalChoicePerson(persontype))
        {
          this.roleinsociety = RoleInSociety.Customer;
          switch (persontype)
          {
        ...
        else
        {
          ++CustomerManager.CustomersInPark_NotWaitingForBus;
          this.roleinsociety = RoleInSociety.Customer;
          this.memberofthepublic = new MemberOfThePublic(persontype, cellblockcontainer, false, player, this.customertype);
          if (persontype >= AnimalType.Protestor1 && persontype <= AnimalType.Protestor6)
          {
    ...
    public void TeleportToJob()
    {
      ...
      this.mechaniccontroller.HasReachedFixableObject();
    }
