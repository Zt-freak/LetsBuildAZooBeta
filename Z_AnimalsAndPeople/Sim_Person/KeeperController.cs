  ...
  internal class KeeperController
  {
      ...
      ref bool IsWalking)
    {
      ...
      if (this.deliveryman.deliveryguystatus == DeliveryGuyStatus.InternalNavigationWaiting)
      {
        ...
        if ((double) this.FeedAnimalsTimerPickPoop <= 1.0 || !OverWorldManager.trashmanager.TryToPickUpPoop(parent.pathnavigator.CurrentTile, this.deliveryman.GetCurrentPenUID()))
          return;
    ...
    public void FinishedWaiting(WalkingPerson parent, ref bool IsWalking)
    {
      if (Z_TrashManager.HasPoopHere(this.deliveryman.GetCurrentPenUID()))
      {
        ...
        this.TryAndTravelToPoop(parent, ref IsWalking);
      }
      else
      {
    ...
    public void TryAndTravelToPoop(WalkingPerson walkingPerson, ref bool IsWalking)
    {
      ...
      ref bool IsWalking)
    {
      ...
      if (this.deliveryman.IsDoingDelivery)
      {
        ...
        {
          if (!this.deliveryman.ReachedTargetLocation(CurrentLocation, ref BlockAutoWalk, ref IsWalking, out bool _))
            return;
          this.FeedAnimalsTimerPickPoop = 0.0f;
