...
namespace TinyZoo.Z_AnimalsAndPeople.Sim_Person
{
  internal class DeliveryManController
  {
      ...
      bool GoToFarm = false)
    {
      ...
      this.DoorLocation = (!GoToFarm ? (ArrayShortCut <= -1 ? player.prisonlayout.cellblockcontainer.GetThisCellBlock(PenUID) : player.prisonlayout.cellblockcontainer.prisonzones[ArrayShortCut]) : player.farms.GetThisFarmFieldByUID(PenUID)).GetGateLocation();
      this.SpaceInfrontOfDoor = new Vector2Int(this.DoorLocation);
      ...
      out bool JobCancelled)
    {
      ...
      if (this.IsDoingDelivery)
      {
        ...
        else if (this.deliveryguystatus == DeliveryGuyStatus.OnWayToJob)
        {
          if (this.TargetHasDoor)
          {
