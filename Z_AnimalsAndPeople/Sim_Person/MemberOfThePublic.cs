...
namespace TinyZoo.Z_AnimalsAndPeople.Sim_Person
{
  internal class MemberOfThePublic
  {
    ...
    private Vector2Int PreOverrideOldShopLocation;
    ...
    private void EndShopQuest(ref bool BlockAutoWalk, ref bool WalkPaused, WalkingPerson parent)
    {
      ...
      if (this.PreOverrideOldQuest != CustomerQuest.Count && this.PreOverrideOldTarget != null)
      {
        this.CurrentQuest = this.PreOverrideOldQuest;
        parent.pathnavigator.TryToGoHere(this.PreOverrideOldTarget, GameFlags.pathset);
        if (this.PreOverrideOldShopLocation != null)
          this.TargetShopLocation = new Vector2Int(this.PreOverrideOldShopLocation);
        this.Ref_WaitingToUseThisShop = this.PREOVERRIDE_Ref_WaitingToUseThisShop;
        if (this.Ref_WaitingToUseThisShop != null)
          this.SetQueuing(parent);
        this.CurrentQuestTarget = new Vector2Int(this.PreOverrideOldTarget);
      }
      this.TempOverrideQuest = CustomerQuest.Count;
    }
      ...
      else
      {
        ...
        else
        {
          if (this.Ref_WaitingToUseThisShop == null && SpeechManager.SomeoneShouldSaySomething && (parent.pathnavigator.CurrentTile.Y < 223 && TinyZoo.Game1.Rnd.Next(0, 100) == 0))
          {
            ...
            Z_NotificationManager.TryAndSaySomething(walkingperson: parent);
          }
    ...
    public string GetThought() => this.customerneeds.GetThought();
      ...
      ref Vector2Int ForceGoHere)
    {
      ...
      else
      {
        if (this.CurrentQuestTarget != null && this.CurrentQuestTarget.CompareMatches(CurrentLocation))
        {
          ...
          else if (this.CurrentQuest == CustomerQuest.WantsToSeeAnimal)
          {
            ...
            if (thisCellBlock != null)
            {
              ...
              if (Player.financialrecords.GetDaysPassed() > 1L)
                Z_NotificationManager.TryAndSaySomething(this.CellUID, parent);
            }
            else
          ...
          else if (this.CurrentQuest == CustomerQuest.SeekingDrink || this.CurrentQuest == CustomerQuest.SeekingFood || (this.CurrentQuest == CustomerQuest.SeekingIceCream || this.CurrentQuest == CustomerQuest.SeekingBuyingSouvenirsBeforeLeavingPark) || (this.CurrentQuest == CustomerQuest.SeekingBench || this.CurrentQuest == CustomerQuest.SeekingBathroom || this.CurrentQuest == CustomerQuest.SeekingBin))
          {
            this.ReachedQuestBuildingRoot(SatisfactionBarAndText.GetQuestToSatisfaction(this.CurrentQuest), ref IsWalking, parent, CurrentLocation, ref BlockAutoWalk);
            return;
          }
    ...
    {
      if (Z_DebugFlags.IsBetaVersion || this.Ref_WaitingToUseThisShop != null || (this.NextQuest != CustomerQuest.Count || this.TempOverrideQuest != CustomerQuest.Count))
        return;
      ...
      if (TileData.IsForFood((TILETYPE) pointOfInterest.W) && this.EatingThis != PersonAttachementType.Count)
        ...
        this.TempOverrideQuest = CustomerQuest.SeekingBuyingSouvenirsBeforeLeavingPark;
      ...
      ShopEntry shopEntry = !player.prisonlayout.layout.BaseTileTypes[MemberOfThePublic.ShopVec.X, MemberOfThePublic.ShopVec.Y - 1].GetIsChild() ? player.shopstatus.GetThisShop(MemberOfThePublic.ShopVec, (TILETYPE) pointOfInterest.W) : player.shopstatus.GetThisShop(player.prisonlayout.layout.BaseTileTypes[MemberOfThePublic.ShopVec.X, MemberOfThePublic.ShopVec.Y - 1].GetParentLocation(), (TILETYPE) pointOfInterest.W);
      if (shopEntry == null || shopEntry.people_usingShop.Count > 0 || shopEntry.people_walkingTo_SHOP.Count > 0)
        return;
      ...
      if (walkingperson.pathnavigator.IsNavigating)
      {
        if (this.TargetShopLocation != null)
          this.PreOverrideOldShopLocation = new Vector2Int(this.TargetShopLocation);
      ...
      WalkingPerson parent)
    {
      ...
      if (this.NextQuest != CustomerQuest.Count && this.CurrentQuest == CustomerQuest.Count)
      {
        if (this.NextQuest == CustomerQuest.SeekingBin)
        {
          ...
          {
            if (Z_GameFlags.Location_Directory.TryAndGetToThis(POINT_OF_INTEREST.Bin, pathnavigator))
            {
              this.CurrentQuestTarget = pathnavigator.GetEndOfCurrentPath();
              this.CurrentQuest = CustomerQuest.SeekingBin;
              return true;
            }
            parent.RemoveAttachment(this.EatingThis);
            Z_TrashManager.DropTrash(parent.pathnavigator.CurrentTile, TrashDrop.GetPersonAttachementTypeToTrash(this.EatingThis));
