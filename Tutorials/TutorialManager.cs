...
namespace TinyZoo.Tutorials
{
  internal class TutorialManager
  {
    ...
    {
      if (this.ztips != null || overworldstate != OverWOrldState.MainMenu || TutorialManager.currenttutorial != TUTORIALTYPE.None)
        return;
      this.TipTimer += DeltaTime;
      if ((double) this.TipTimer <= 45.0)
        return;
      this.TipTimer = 0.0f;
      Z_NotificationManager.RemoveThese(Z_NotificationType.TicketPrice);
      Expensiveness expensiveness = AnimalTicketValue.GetTicketExpensivenss(player);
      if (FeatureFlags.BlockTicketPrice)
        expensiveness = Expensiveness.AboutRight;
      if (player.breeds.QueickCheckSomethingFinished(player))
      {
        if (this.LAST_ztiptype == ZTipType.AnimalsBreedReady)
        {
          if (!this.SHouldSHowRepeatTip())
            return;
          this.ztips = new ZTipManager(ZTipType.AnimalsBreedReady, ref this.LAST_ztiptype, ref this.ZTIPCYCLE);
        }
        else
        {
          this.ZTIPRepeats = 0;
      ...
      else
      {
        switch (expensiveness)
        {
          case Expensiveness.TooCheap:
            if (this.LAST_ztiptype == ZTipType.TicketPriceTooCheap)
            {
              if (!this.SHouldSHowRepeatTip())
                break;
              Z_NotificationManager.AddNotificationPackage(new NotificationPackage(Z_NotificationType.TicketPrice, ZTipType.TicketPriceTooCheap), player);
              break;
            }
            Z_NotificationManager.AddNotificationPackage(new NotificationPackage(Z_NotificationType.TicketPrice, ZTipType.TicketPriceTooCheap), player);
            break;
          case Expensiveness.TooExpensive:
            if (this.LAST_ztiptype == ZTipType.TicketPriceTooExpensive)
            {
              if (!this.SHouldSHowRepeatTip())
                break;
              Z_NotificationManager.AddNotificationPackage(new NotificationPackage(Z_NotificationType.TicketPrice, ZTipType.TicketPriceTooExpensive), player);
              break;
            }
            Z_NotificationManager.AddNotificationPackage(new NotificationPackage(Z_NotificationType.TicketPrice, ZTipType.TicketPriceTooExpensive), player);
            break;
