  ...
  internal class CRISPRManager
  {
    ...
    public bool UpdateCRISPRManager(Player player, float DeltaTime)
    {
      ...
      if (this.selectSpecies != null)
      {
        ...
        {
          player.crisprBreeds.AddBreedToCRISPRBuilding(this.ref_Building.BuildingUID, animalTypeArray[0], animalTypeArray[1]);
          this.selectSpecies.LerpOff();
          this.CreateMainPanels(player);
        }
      }
      bool GoToManage;
      int index = this.mainPanel.UpdateCRISPRMainPanel(player, DeltaTime, Vector2.Zero, out Cancel, out GoToManage);
      if (index > -1)
      {
        if (GoToManage)
        {
          this.manageActive = new CRISPR_ManageActivePanel(this.ref_Building.crisprBreeds[index], this.BaseScale);
          this.manageActive.location = new Vector2(512f, 384f);
          this.mainPanel.LerpOff();
        }
        else
        {
          this.selectSpecies = new CRISPR_SelectSpecies(player, this.BaseScale);
