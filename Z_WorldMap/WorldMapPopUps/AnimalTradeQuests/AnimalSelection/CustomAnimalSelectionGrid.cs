...
namespace TinyZoo.Z_WorldMap.WorldMapPopUps.AnimalTradeQuests.AnimalSelection
{
  internal class CustomAnimalSelectionGrid
  {
    ...
    private List<PrisonerInfo> refAnimals;
    ...
    private void GoToThisPage(int pageIndex)
    {
      ...
      else
      {
        for (int startIndex = this.startIndex; startIndex < this.endIndex; ++startIndex)
        {
          AlienEntry alienEntry1 = new AlienEntry(this.refAnimals[startIndex].GetAnimalPainted(), true, true, this.refAnimals[startIndex].intakeperson.CLIndex, this.BaseScale, this.refAnimals[startIndex].intakeperson.HeadType, this.refAnimals[startIndex].intakeperson.HeadVariant);
          ...
          if (this.AddSellPrice)
          {
            ...
            alienEntry1.AddStringBelow_NEW("$" + (object) AnimalSellCostCalculator.GetSellCostOfPlayerAnimal(this.refAnimals[startIndex]), ColourData.Z_Cream, this.BaseScale, AssetContainer.SpringFontX1AndHalf, RecreateIfExists_ElseUpdateString: false);
          }
      ...
      Vector2 offset)
    {
      ...
      for (int startIndex = this.startIndex; startIndex < this.endIndex; ++startIndex)
      {
        if (this.alienEntries[startIndex].UpdateAlienEntry(offset, DeltaTime, player))
        {
          this.lastSelectedIndex = startIndex;
          return this.refAnimals[startIndex];
        }
      }
