...
namespace TinyZoo.Z_SummaryPopUps.People.Employee.EmployeeView
{
  internal class EmploymentPanel
  {
    ...
    public EmploymentPanel(WalkingPerson walkingperson_, float basescale_, float forceToThisWidth = -1f)
    {
      ...
      this.slider = new SalarySliderWithAverage(this.basescale, (float) minimumWage, (float) maximumWage, (float) quickemplyeedescription.CurrentSalary, this.scalehelper.ScaleX(140f), true, quickemplyeedescription.AgencyPay_WeeksLeft > 0, (float) num);
