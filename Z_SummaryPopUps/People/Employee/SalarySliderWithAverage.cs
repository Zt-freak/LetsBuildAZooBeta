...
namespace TinyZoo.Z_SummaryPopUps.People.Employee
{
  internal class SalarySliderWithAverage
  {
      ...
      string label = "Salary")
    {
      ...
      {
        this.totaltext = new ZGenericText("Total Salary Cost: " + (this.salary * HiringAgency.GetPercentageAgencyCutAsFloat()).ToString("f0"), this.basescale, false);
        this.agencytext = new ZGenericText("(Agency Pay Extra 30%)", this.basescale, false);
      }
