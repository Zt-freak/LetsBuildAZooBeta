...
namespace TinyZoo.PlayerDir.Shops
{
  internal class ShopEntry
  {
    ...
    public void OnDestroyThisShop(Player player)
    {
      ...
      player.employees.openPositionsContainer.RemoveThisOpenPosition(this.ShopUID);
    }

    public void OnMoveThisShop(Vector2Int Loction)
    {
      for (int index = 0; index < this.employeeshere.Count; ++index)
