...
namespace TinyZoo.PlayerDir.Layout.CellBlocks.Pen_Items
{
  internal class PenItems
  {
    ...
    public int GetWaterSize(Player player)
    {
      ...
      if (flag && num == 0 && !Z_DebugFlags.IsBetaVersion)
        Z_NotificationManager.AddNotificationPackage(new NotificationPackage(Z_NotificationType.A_NoWaterConnection), player);
      return num;
    }
