...
namespace TinyZoo.PlayerDir.CRISPR
{
  ...
  {
    private List<CRISPRBuilding> CRISPRBuildings;
      ...
      AnimalType animalTwo)
    {
      for (int index = 0; index < this.CRISPRBuildings.Count; ++index)
      {
        if (this.CRISPRBuildings[index].BuildingUID == buildingUID)
        {
          ...
          this.CRISPRBuildings[index].AddGenomePair(breed);
        }
      }
    }
    ...
    public CRISPR_Breed(Reader reader)
    {
      ...
      for (int index = 0; index < _out; ++index)
        this.CRISPRBuildings.Add(new CRISPRBuilding(reader));
