...
namespace TinyZoo.PlayerDir.HeroQuests
{
  internal class HeroProgressStatus
  {
    ...
    public string GetNextQustUnlockText(Player player)
    {
      ...
      return nextQuest.UnlockThisQuestCriteria.CheckIsComplete(player) ? SEngine.Localization.Localization.GetText(759) : SEngine.Localization.Localization.GetText(760) + nextQuest.UnlockThisQuestCriteria.GetObjectiveHeading(out string _, player, out int _, out int _);
    }
    ...
    public bool TryAndUnlockNextQuest(Player player)
    {
      if (!this.IsUnlocked)
      {
        if (!this.CompletedQuests.Contains(this.RefHeroQuestPack.heroquests[0].UID) && this.RefHeroQuestPack.heroquests[0].UnlockThisQuestCriteria.CheckIsComplete(player))
        {
          this.CurrentQuestIndex = this.RefHeroQuestPack.heroquests[0].UID;
          this.IsNew = true;
          this.IsUnlocked = true;
          if (this.RefHeroQuestPack.heroquests[0].tutorialquestspecial == TutorialQuestSpecial.UnlockAbilityToBuyLand)
      ...
      else
      {
        for (int index = 0; index < this.RefHeroQuestPack.heroquests.Count; ++index)
        {
          if (!this.CompletedQuests.Contains(this.RefHeroQuestPack.heroquests[index].UID))
          {
            if (!this.RefHeroQuestPack.heroquests[index].UnlockThisQuestCriteria.CheckIsComplete(player))
              return false;
