  ...
  internal class BusPeopleAndRoutes
  {
    ...
    public int GetPeopleForThisBus(int MaximumPeopleOnThisBus, bool IsLastBus)
    {
      ...
      if (Player.financialrecords.GetDaysPassed() == 0L && this.CollectionsToday == 2)
        NewCustomerCalculator.AddVIPonTheFly(new VIP_Entry(AnimalType.SpecialEvent_Scientist, CustomerType.ResearchGrantGuy, CriticalChoiceAction.GetCriticalChoiceCharacterToString(CriticalChoiceCharacter.Scientist)));
