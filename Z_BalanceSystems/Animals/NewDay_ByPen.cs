  ...
  internal class NewDay_ByPen
  {
    ...
    internal static void CalcAnimals(Player player)
    {
      ...
      for (int index1 = 0; index1 < player.prisonlayout.cellblockcontainer.prisonzones.Count; ++index1)
      {
        ...
        float aniamlsInThisPen = player.prisonlayout.cellblockcontainer.prisonzones[index1].GetSpaceRequiredByAniamlsInThisPen();
        ...
        if ((double) aniamlsInThisPen > (double) floorSpaceVolume)
          NewDay_ByPen.Day_TotalAnimalScore_UnderSpace += aniamlsInThisPen - floorSpaceVolume;
        NewDay_ByPen.Day_TotalSpace += floorSpaceVolume;
