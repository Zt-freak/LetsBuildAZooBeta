...
namespace TinyZoo.PathFinding.HierarchicalPathFinding
{
  internal class HierarchicalPathFind
  {
      ...
      bool delayRepathing = true)
    {
      ...
      if (hierarchicalnode.neighbours.Count == 0)
      {
      ...
      bool randomedgenode = true)
    {
      ...
      if (startNode.supernode != null && targetNode.supernode != null)
      {
        ...
        if (pathRecursive == null)
          return (List<HierarchicalNode>) null;
        HierarchicalNode hierarchicalNode = startNode.supernode;
        foreach (HierarchicalNode key1 in pathRecursive)
        {
          ...
          foreach (HierarchicalNode other in hierarchicalNode.neighbours[key1])
          {
            float num2 = !randomedgenode || this.preferstraight ? (!this.preferstraight ? startNode1.GetDistanceEuclidean(other) + 0.5f * targetNode.GetDistanceEuclidean(other) : startNode1.GetDistanceEuclidean(other)) : (float) Game1.Rnd.NextDouble();
          ...
          if (hierarchicalNode.paths.ContainsKey(key2))
          {
            hierarchicalNodeList1.AddRange((IEnumerable<HierarchicalNode>) hierarchicalNode.paths[key2]);
          ...
          foreach (HierarchicalNode key3 in targetNode1.neighbours.Keys)
          {
            if (key3.supernode == key1)
            {
              hierarchicalNodeList1.Add(key3);
              break;
            }
          }
          hierarchicalNode = key1;
      ...
      DiagonalsAllowed diagonalsAllowed = DiagonalsAllowed.NO_DIAGONALS)
    {
      ...
      while (HierarchicalPathFind.openSet.Count > 0)
      {
        ...
        {
          if (!hierarchicalNodeSet.Contains(key) && (!withinCluster || key.cluster == hierarchicalNode.cluster))
          {
            int num1 = diagonalsAllowed == DiagonalsAllowed.ALLOW_DIAGONALS ? 1 : (diagonalsAllowed == DiagonalsAllowed.ALLOW_UNBLOCKED ? 1 : 0);
            float num2 = hierarchicalNode.gCost + hierarchicalNode.GetDistanceManhattan(key);
            if (hierarchicalNode.parent != null)
            {
              Vector2 vector2_1 = key.location - hierarchicalNode.location;
            ...
            if (!HierarchicalPathFind.openSet.Contains(key))
            {
              key.gCost = num2;
              key.hCost = key.GetDistanceEuclidean(targetNode);
              key.fCost = key.gCost + key.hCost;
              key.parent = hierarchicalNode;
              HierarchicalPathFind.openSet.Add(key);
            }
            else if ((double) num2 < (double) key.gCost)
            {
              key.gCost = num2;
              key.parent = hierarchicalNode;
              key.fCost = key.gCost + key.hCost;
              HierarchicalPathFind.openSet.UpdateItem(key.binaryHeapIndex);
            }
          }
        }
      }
