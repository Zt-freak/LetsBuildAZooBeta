  ...
  internal class DayNightManager
  {
    ...
    public void UpdateDayNightManager(Player player, float DeltaTime, float SimulationTime)
    {
      ...
      if (this.AllowPressDayNightButton())
      {
        ...
        if ((this.startdybutton.UpdateStartDayButton(player, DeltaTime) || this.ControllerSelected && player.inputmap.PressedThisFrame[26] || Z_GameFlags.HasPassedMidnight && !this.HasStartedDay && (double) Z_GameFlags.DayTimer >= (double) Z_GameFlags.ZooOpenTime_InSeconds) && !TrailerDemoFlags.DisableSave)
        {
          SoundEffectsManager.PlaySpecificSound(SoundEffectType.ConfirmClick);
