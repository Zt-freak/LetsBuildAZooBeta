...
namespace TinyZoo.OverWorld.OverWorldEnv.Customers
{
  internal class WalkingPerson : AnimatedGameObject
  {
    ...
    public WalkingPerson(AnimalRenderMan _animalrenderer) => this.animalrenderer = _animalrenderer;

    public WalkingPerson(DynamicEnrichmentItem _dynamicenrichmentitem) => this.dynamicenrichmentitem = _dynamicenrichmentitem;
      ...
      int ShopUID = -1)
    {
      ...
      else if (this.simperson.Ref_Employee != null && this.simperson.Ref_Employee.employeetype == EmployeeType.Keeper)
        _MovementSpeed = 1.5f;
    ...
    public bool UpdateWalkingPerson(float DeltaTime, Player player, out bool RemoveThisPerson)
    {
      ...
      if (this.simperson.roleinsociety != RoleInSociety.Avatar || OverWorldManager.overworldstate != OverWOrldState.PlayingAsAvatar && !Z_GameFlags.IsWaitingToReturnToControllerWalk)
      {
        ...
        if (!this.WalkPaused)
        {
          if (Z_GameFlags.breakoutAnimals.Count > 0 && (this.simperson.roleinsociety != RoleInSociety.Employee || !Z_DebugFlags.EmployeesCannotBeKilledByAnimals) && (this.simperson.roleinsociety != RoleInSociety.Avatar && (this.simperson.roleinsociety != RoleInSociety.Employee || this.simperson.Ref_Employee.employeetype != EmployeeType.Police)))
          {
            ...
            {
              if (this.pathnavigator.CurrentTile.CompareMatches(Z_GameFlags.breakoutAnimals[index].Location))
              {
                this.KillThisPerson();
                return true;
              }
      ...
      if ((double) player.player.touchinput.ReleaseTapArray[0].X > 0.0)
      {
        ...
        screenSpace.Y -= (float) ((double) Sengine.WorldOriginandScale.Z * (double) Sengine.ScreenRatioUpwardsMultiplier.Y * (double) this.scale * ((double) this.DrawRect.Height * 0.5));
        if (MathStuff.CheckPointCollision(true, screenSpace, this.scale * Sengine.WorldOriginandScale.Z, (float) this.DrawRect.Width, (float) this.DrawRect.Height * Sengine.ScreenRatioUpwardsMultiplier.Y, player.player.touchinput.ReleaseTapArray[0]))
          OverWorldManager.zoopopupHolder.CreateZooPopUps(this, player);
      }
      if (this.simperson.roleinsociety == RoleInSociety.Customer)
    ...
    {
      int maxValue = 0;
      if (this.pathnavigator.CurrentTile.Y > 0 && !Z_GameFlags.pathfinder.GetIsBlocked(this.pathnavigator.CurrentTile.X, this.pathnavigator.CurrentTile.Y - 1))
        ++maxValue;
      if (this.pathnavigator.CurrentTile.X > 0 && !Z_GameFlags.pathfinder.GetIsBlocked(this.pathnavigator.CurrentTile.X - 1, this.pathnavigator.CurrentTile.Y))
        ++maxValue;
      if (this.pathnavigator.CurrentTile.X < TileMath.GetOverWorldMapSize_XDefault() - 1 && !Z_GameFlags.pathfinder.GetIsBlocked(this.pathnavigator.CurrentTile.X + 1, this.pathnavigator.CurrentTile.Y))
        ++maxValue;
      if (this.pathnavigator.CurrentTile.Y < TileMath.GetOverWorldMapSize_YSize() - 1 && !Z_GameFlags.pathfinder.GetIsBlocked(this.pathnavigator.CurrentTile.X, this.pathnavigator.CurrentTile.Y + 1))
        ++maxValue;
      if (maxValue <= 0)
        return;
      if (maxValue == 1)
      ...
      if (this.pathnavigator.CurrentTile.Y > 0 && this.CameFromHere != DirectionPressed.Up && !Z_GameFlags.pathfinder.GetIsBlocked(this.pathnavigator.CurrentTile.X, this.pathnavigator.CurrentTile.Y - 1))
      {
        if (num1 == 0)
        {
      ...
      if (this.pathnavigator.CurrentTile.X > 0 && this.CameFromHere != DirectionPressed.Left && !Z_GameFlags.pathfinder.GetIsBlocked(this.pathnavigator.CurrentTile.X - 1, this.pathnavigator.CurrentTile.Y))
      {
        if (num1 == 0)
        {
      ...
      if (this.pathnavigator.CurrentTile.X < TileMath.GetOverWorldMapSize_XDefault() - 1 && this.CameFromHere != DirectionPressed.Right && !Z_GameFlags.pathfinder.GetIsBlocked(this.pathnavigator.CurrentTile.X + 1, this.pathnavigator.CurrentTile.Y))
      {
        if (num1 == 0)
        {
          locations.Add(new PathNode(this.pathnavigator.CurrentTile.X + 1, this.pathnavigator.CurrentTile.Y));
      ...
      if (this.pathnavigator.CurrentTile.Y >= TileMath.GetOverWorldMapSize_YSize() - 1 || this.CameFromHere == DirectionPressed.Down || Z_GameFlags.pathfinder.GetIsBlocked(this.pathnavigator.CurrentTile.X, this.pathnavigator.CurrentTile.Y + 1))
        return;
      if (num1 == 0)
      {
        locations.Add(new PathNode(this.pathnavigator.CurrentTile.X, this.pathnavigator.CurrentTile.Y + 1));
    ...
    public void DrawWalkingPerson()
    {
      ...
      else
      {
        ...
        if (this.statusicon == null || this.statusicon.statusicontype == StatusIconType.None)
          return;
        this.statusicon.DrawStatusIcon(this.vLocation);
      }
    }
