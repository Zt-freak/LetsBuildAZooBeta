...
namespace TinyZoo.OverWorld.SelectCell
{
  internal class CellInfo
  {
    ...
    private void Create(string Name, string Description)
    {
      ...
      this.simpletext = new SimpleTextHandler(Description, this.VSCale.X - this.scaleHelper.DefaultBuffer.X, true, this.BaseScale, true, true);
      ...
      this.UseButton.vLocation.Y = (float) ((double) vector2.Y * 0.5 - (double) this.scaleHelper.DefaultBuffer.Y - (double) this.UseButton.GetSize_True().Y * 0.5);
      this.cellclose = new BackButton(true, BaseScale: this.BaseScale);
      this.cellclose.vLocation.X = (float) ((double) vector2.X * 0.5 - (double) this.scaleHelper.DefaultBuffer.X - (double) this.cellclose.GetSize().X * 0.5);
      this.cellclose.vLocation.Y = (float) (-(double) vector2.Y * 0.5 + (double) this.scaleHelper.DefaultBuffer.Y + (double) this.cellclose.GetSize().Y * 0.5);
