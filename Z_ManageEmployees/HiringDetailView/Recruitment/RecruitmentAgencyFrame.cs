...
namespace TinyZoo.Z_ManageEmployees.HiringDetailView.Recruitment
{
  internal class RecruitmentAgencyFrame
  {
    ...
    public RecruitmentAgencyFrame(float forceWidth, float BaseScale)
    {
      ...
      this.explanationText = new SimpleTextHandler(string.Format("Instantly find and hire employees through a Recruitment Agency, which will take a {0}% agency fee for {1} weeks.", (object) (float) ((double) HiringAgency.GetPercentageAgencyCutAsFloat() * 100.0), (object) HiringAgency.GetLengthOfAgencyCut_Weeks()), true, (float) ((double) forceWidth / 1024.0 * 0.899999976158142), BaseScale);
