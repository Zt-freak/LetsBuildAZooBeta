...
namespace TinyZoo.Z_ManageEmployees.HiringDetailView.Applicants.Rows
{
  internal class ApplicantRow
  {
        ...
        else
        {
          switch (index)
          {
            ...
            case 3:
              ...
              if (isAgencyHire)
              {
                this.agencySalaryText = new ZGenericText(string.Format("(+{0}%)", (object) (float) ((double) HiringAgency.GetPercentageAgencyCutAsFloat() * 100.0)), BaseScale);
                this.agencySalaryText.vLocation.X = num4 - num2;
                this.agencySalaryText.vLocation.Y += this.salaryText.GetSize().Y * 0.5f;
                this.salaryText.vLocation.Y -= this.agencySalaryText.GetSize().Y * 0.5f;
                break;
              }
              break;
            case 4:
      ...
      bool BlockButtonClick = false)
    {
      ...
      if ((double) this.slidingLerper.Value != 0.0)
