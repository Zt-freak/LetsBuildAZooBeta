...
namespace TinyZoo.Z_ManageEmployees.HiringSummary
{
  internal class HiringQuickInfoFrame
  {
      ...
      float ForceWidth = -1f)
    {
      ...
      switch (this.recruitmentInfo)
      {
        case RecruitmentInfoType.JobPostings:
          ...
          TextToWrite = TextToWrite + "Current Openings: " + (object) num3 + "~Cost Per Day: $" + (object) num4;
      ...
      {
        TextButton textButton1 = new TextButton(BaseScale, this.GetButtonString(this.buttonTypes[index]), Length, _OverrideFrameScale: BaseScale);
      ...
      bool flag3 = _infoType == RecruitmentInfoType.HiringAgency;
      if (!(flag1 | flag3))
        return;
      if (flag3)
    ...
    private string GetButtonString(RecruitmentButtonType buttonType)
    {
      switch (buttonType)
      {
