...
namespace TinyZoo.Z_ManageEmployees
{
  internal class ManageEmployeeDisplayData
  {
    ...
    public static BuildingSummaryData GetBuildingSummaryDataForThis(
      ...
      ShopEntry shopEntry,
      out bool HasData,
      EmployeeType RoamingEmployeeType = EmployeeType.Count)
    {
      HasData = true;
      List<Employee> employeesInThisShop = RoamingEmployeeType == EmployeeType.Count ? player.employees.GetEmployeesInThisSpecificShop(shopEntry.ShopUID) : player.employees.GetEmployeesOfThisType(RoamingEmployeeType);
      BuildingSummaryData buildingSummaryData = new BuildingSummaryData();
      if (RoamingEmployeeType == EmployeeType.Count && shopEntry != null)
    ...
    public static PerformanceSummaryData GetEmployeePerformanceDataForThis(
      ...
      Player player,
      ShopEntry refShop,
      EmployeeType RoamingEmplyeeType = EmployeeType.Count,
      bool GetDataForYesterday = false)
    {
      PerformanceSummaryData performanceSummaryData = new PerformanceSummaryData();
      List<Employee> employeeList = new List<Employee>();
      if (filterType == EmployeeFilterType.ThisBranch && RoamingEmplyeeType != EmployeeType.Count)
