...
namespace TinyZoo.Z_ManageEmployees.EmployeeView
{
  internal class EmployeePerformancePanel
  {
      ...
      EmployeeType _RoamingEmployeeType = EmployeeType.Count)
    {
      ...
      if (_RoamingEmployeeType != EmployeeType.Count)
      {
      ...
      string text = (_RoamingEmployeeType == EmployeeType.Count ? (object) (empty + TileData.GetTileStats(shopEntry.tiletype).Name) : (object) (empty + Employees.GetEmployeeThypeToString(_RoamingEmployeeType))).ToString() + ": " + (object) employees.Count;
      if (num != -1)
