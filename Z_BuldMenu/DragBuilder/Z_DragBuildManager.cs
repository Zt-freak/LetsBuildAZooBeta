  ...
  internal class Z_DragBuildManager
  {
        ...
        if (!this.block.SomethingIsBlocked && !this.HasNOTMovedSelectionSincePurchase)
        {
          if ((double) player.player.touchinput.ReleaseTapArray[0].X > 0.0 && (!MouseBlockedByUI || this.IsMove))
          {
            ...
            if (player.Stats.SpendCash(SpendThis, SpendingCashOnThis.BuyBuilding, player))
            {
              ...
              {
                player.prisonlayout.BuildTileFromTileRenderer(this.tilerenderer, player.livestats.consumptionstatus, player);
              ...
              {
                OverWorldManager.quickpickemployeemanager = new QuickPickEmployeeManager(this.tilerenderer.tiletypeonconstruct, Shop_UID);
      ...
      bool ForceRefreshFootPrint)
    {
      ...
      if (!spaceToTileLocation.CompareMatches(this.LastLocation) | ForceRefreshFootPrint)
      {
        ...
        this.block = this.decoratethisprisonzone == null ? wallsandfloors.CanBuildThisHere(this.tilerenderer.TileLocation, this.tilerenderer, this.IsFloorLayer, layoudata: player.prisonlayout.layout) : wallsandfloors.CanBuildThisHere(this.tilerenderer.TileLocation, this.tilerenderer, this.IsFloorLayer, true, this.decoratethisprisonzone, this.IsPenWater);
