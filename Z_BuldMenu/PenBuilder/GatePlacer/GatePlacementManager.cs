  ...
  internal class GatePlacementManager
  {
      ...
      Player player)
    {
      ...
      if (Z_GameFlags.ForceToNewScreen == ForceToNewScreen.MovePen)
      {
        ...
        this.penmovercollision = new PenMoveCollision(this.tilerenderers, perimeterBuilder, this.prisonzone_ForMove);
    ...
    public void SetMouseLocation(Vector2Int TileLoc, PerimeterBuilder perimeterBuilder)
    {
      if (this.IsMoveWHolePen)
      {
        ...
        this.penmovercollision.SetNewLocations(TileLoc - this.MOVE_StartMiddle);
      }
      else
      {
      ...
      AnimalsInPens peopleandbeams)
    {
      if (GameFlags.IsUsingController && !this.IsMoveWHolePen)
      {
        ...
        {
          this.SetMouseLocation(this.gateplacementcontols.GetSelectedLocation(), perimeterBuilder);
