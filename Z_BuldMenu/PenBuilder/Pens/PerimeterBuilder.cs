...
namespace TinyZoo.Z_BuldMenu.PenBuilder.Pens
{
  internal class PerimeterBuilder
  {
    ...
    public void SetMouseLocation(Vector2Int TileLoc, bool ForceRemake)
    {
      ...
      if (this.CommitedBuildTiles.Count > 0)
      {
        ...
        if (TileMath.TileIsInWorld(TileLoc.X, TileLoc.Y) && !TileLoc.CompareMatches(this.CommitedBuildTiles[0].TileLocation) && pathNavigator.TryToGoHereSquare(TileLoc, GameFlags.pathset, hierarchy: Z_GameFlags.pathfinder.hierachicalPathFind))
        {
          ...
          if (currentPath != null)
          {
            Console.WriteLine("NODES" + (object) currentPath.Count);
      ...
      Console.WriteLine("MOUSE" + (object) this.MousePointer.TileLocation.X + "Y" + (object) this.MousePointer.TileLocation.Y);
    }

    public List<Vector2Int> GeturrentAnimalFloorSpace(bool IncludeMouseAndUncommited = false) => PerimeterBuilderHelper.GeturrentAnimalFloorSpace(this.CommitedBuildTiles, this.Uncommited, IncludeMouseAndUncommited);
      ...
      out bool TriedToBuyButCouldNotAfford)
    {
      ...
      if (flag1)
        this.SetMouseLocation(this.MousePointer.TileLocation, true);
      for (int index = 0; index < this.Uncommited.Count; ++index)
