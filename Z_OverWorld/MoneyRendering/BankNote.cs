...
namespace TinyZoo.Z_OverWorld.MoneyRendering
{
  internal class BankNote : GameObject
  {
    ...
    public void DrawBankNote()
    {
      ...
      if (this.poptype == IconPopType.Damage)
        TextFunctions.DrawJustifiedText(this.Value, 2f * this.poper.CurrentValue, screenSpace + new Vector2(0.0f, -17f * this.poper.CurrentValue) + new Vector2(this.HorizontalDrift * 0.01f, (float) (-(double) this.VerticalDrift * 0.00999999977648258)), Color.White, this.fAlpha, AssetContainer.PixelNumWithBlackOutline, AssetContainer.pointspritebatch01);
      else
        TextFunctions.DrawJustifiedText(this.Value, 0.5f * this.poper.CurrentValue, screenSpace + new Vector2(0.0f, -17f * this.poper.CurrentValue), this.TextCOlour, this.fAlpha, AssetContainer.roundaboutFont, AssetContainer.pointspritebatch01, true);
    }
  }
}
