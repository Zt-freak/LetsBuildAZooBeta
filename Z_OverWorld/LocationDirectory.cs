...
namespace TinyZoo.Z_OverWorld
{
  internal class LocationDirectory
  {
    ...
    public LocationDirectory(int CityWidth, int CityHeight, int _BlockSize)
    {
      this.TotalPointsOfInterestByType = new int[10];
      this.BlockSize = _BlockSize;
      this.CityGrid = new Loc_CityBlock[(CityWidth + _BlockSize) / this.BlockSize, (CityHeight + _BlockSize) / _BlockSize];
      for (int index1 = 0; index1 < this.CityGrid.GetLength(0); ++index1)
      {
        for (int index2 = 0; index2 < this.CityGrid.GetLength(1); ++index2)
          this.CityGrid[index1, index2] = new Loc_CityBlock(_BlockSize, new Vector2Int(index1 * this.BlockSize, index2 * this.BlockSize));
      }
    }
