...
namespace TinyZoo.Z_HUD.Z_Notification
{
  internal class ButtHolder
  {
    ...
    public bool UpdateButtHolder(ref int ActiveBeforeThis, float DeltaTime, Player player)
    {
      ...
      {
        if (notificationpackage.AlertStatus == NotificationAlertStatus.Exclamation || notificationpackage.AlertStatus == NotificationAlertStatus.Danger || (notificationpackage.AlertStatus == NotificationAlertStatus.Special_Heart || notificationpackage.AlertStatus == NotificationAlertStatus.Special_Star))
          ++this.alertcount;
      }
      return flag1;
    }
    ...
    public void DrawButtHolder(float XLOX)
    {
      ...
      TextFunctions.DrawTextWithDropShadow(this.alertcount.ToString(), 2f, vector2 + new Vector2(16f, -8f), Color.White, 1f, AssetContainer.springFont, AssetContainer.pointspritebatch03, false);
