...
namespace TinyZoo.Z_HUD.PointAtThings
{
  internal class PointOffScreenManager
  {
    ...
    private void Processotification(Player player)
    {
      ...
      switch (PointOffScreenManager.notificationstotrack[0].notificationtotrack[0].notificationtype)
      {
        ...
        case Z_NotificationType.A_CRISPR_HybridBirth:
          for (int index1 = 0; index1 < PointOffScreenManager.notificationstotrack.Count; ++index1)
          {
            for (int index2 = 0; index2 < PointOffScreenManager.notificationstotrack[index1].notificationtotrack.Count; ++index2)
            {
              ...
              PointOffScreenManager.AddPointer(player.crisprBreeds.GetBuildingForThisBreed(notificationPackage.AnimalOrPenUID).Location + new Vector2Int(0, -2), OffscreenPointerType.HybridBirth);
            }
          }
          ZHudManager.zquestpins.PinRibbonTrackView(OffscreenPointerType.HybridBirth, player);
          break;
        ...
        case Z_NotificationType.F_AJobHasApplicants:
          ...
          {
            if (applicantsOpenPositions[index].RoamingEmployeeType != EmployeeType.Count)
            {
              if (applicantsOpenPositions[index].RoamingEmployeeType != EmployeeType.Keeper && !flag2)
              {
                flag2 = true;
                flag1 = true;
            ...
            {
              ShopEntry shopEntry = !TileData.IsAnArchitectOffice(applicantsOpenPositions[index].tileType) ? (!TileData.IsThisAFacility(applicantsOpenPositions[index].tileType) ? player.shopstatus.GetThisShop(applicantsOpenPositions[index].ShopUID) : player.shopstatus.GetThisFacility(applicantsOpenPositions[index].ShopUID)) : player.shopstatus.GetThisArchitectsOffice(applicantsOpenPositions[index].ShopUID);
              if (shopEntry != null)
              {
                flag1 = true;
    ...
    internal static void DoubleCheckPointAtThis(Vector2Int location, TILETYPE tile)
    {
      if (PointOffScreenManager.QuestPointer == null)
        return;
      PointOffScreenManager.QuestPointer.SetTargetLocation(TileMath.GetTileToWorldSpace(location));
    }
    ...
    internal static bool QuestPointerIsActive(BuildingManageButton thistype = BuildingManageButton.Count)
    {
      switch (thistype)
      {
        case BuildingManageButton.Pen_AddItemsToPen:
          for (int index = PointOffScreenManager.eventpointers.Count - 1; index > -1; --index)
          {
            if (PointOffScreenManager.eventpointers[index].offscreenPointerType == OffscreenPointerType.PointAtNoWater || PointOffScreenManager.eventpointers[index].offscreenPointerType == OffscreenPointerType.NoEnrichment)
              return true;
          }
          break;
