...
namespace TinyZoo.Z_ManagePen.Buttons
{
  internal class Z_BreakoutButton
  {
    ...
    public Z_BreakoutButton(BreakOutButtonType buttontype, float Lngth = 150f, float _OVERALLMULT = 1f)
    {
      ...
      this.VSCALE = new Vector2(Lngth, 40f * this.OVERALLMULT);
    ...
    public void DrawZ_BreakoutButton(Vector2 Offset, SpriteBatch spritebatch, float ShrinkValue = 0.0f)
    {
      ...
      this.gobject.DrawGameObjectNineSlice(spritebatch, AssetContainer.SpriteSheet, Offset, this.VSCALE * num);
      ...
      if (!this.MouseOver)
        return;
      this.MouseOverObj.DrawGameObjectNineSlice(spritebatch, AssetContainer.SpriteSheet, Offset, this.VSCALE * num);
    }
  }
}
