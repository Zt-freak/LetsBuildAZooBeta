      ...
      OverWorldEnvironmentManager overworldenvironment)
    {
      ...
      switch (buildingManageButton)
      {
        case BuildingManageButton.Move:
          ...
          OverWorldManager.movebuilding = new Z_MoveBuildingManager(SellingThislayout, this.penitem.Location, player, overworldenvironment, prisonzone, true);
          OverWorldManager.overworldstate = OverWOrldState.MoveBuilding;
          Z_SellStructureManager.IsMove = false;
          break;
        case BuildingManageButton.Destroy:
          ...
          else
          {
            prisonzone.penItems.RemoveItem(this.penitem.Location, this.penitem.tiletype, prisonzone.Cell_UID, player);
            int CashRefund = 0;
            SellUIManager.DestroyBuildingStatic(player, OverWorldManager.overworldenvironment, new LayoutEntry(this.penitem.tiletype), this.penitem.Location, ref CashRefund);
          }
          return true;
      }
      return false;
