  ...
  internal class Z_GameFlags
  {
    ...
    internal static float GetBaseScaleForUI()
    {
      if ((double) PlayerStats.UXMult <= 0.0)
        PlayerStats.UXMult = (double) SEngine.Game1.ScreenResolution.X <= 3000.0 ? ((double) SEngine.Game1.ScreenResolution.X <= 1600.0 ? 2f : 3f) : 4f;
    ...
    internal static float GetTimeThatParkOpensInMorning() => Z_GameFlags.ZooOpenTime_InSeconds;

    internal static float GetInGameSecondsPerHour() => Z_GameFlags.SecondsPerHour;

    internal static string GetTimeAsStringFromMinutes(int TotalMinutes)
    {
      double secondsPerMinute = (double) Z_GameFlags.SecondsPerMinute;
      return Z_GameFlags.FormatFloatToTime(Z_GameFlags.DayTimer / Z_GameFlags.SecondsInDay, true);
    }
    ...
    {
      if (OverWorldManager.overworldstate == OverWOrldState.Build || TrailerDemoFlags.HasTrailerFlag)
        return true;
