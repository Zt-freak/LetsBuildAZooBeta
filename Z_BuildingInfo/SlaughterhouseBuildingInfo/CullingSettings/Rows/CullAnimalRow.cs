...
namespace TinyZoo.Z_BuildingInfo.SlaughterhouseBuildingInfo.CullingSettings.Rows
{
  internal class CullAnimalRow
  {
    ...
    public CullAnimalRow(AnimalType animalType, Player player, float BaseScale)
    {
      ...
      this.animalInFrame.Location = vector2_1;
      this.animalInFrame.Location += this.animalInFrame.GetSize() * 0.5f;
      vector2_1.X += this.animalInFrame.GetSize().X;
      vector2_1.X += defaultBuffer.X;
      this.name.vLocation = vector2_1;
      vector2_1.Y += this.name.GetSize().Y;
      this.dragBarLabel.vLocation = vector2_1;
      vector2_1.Y += this.dragBarLabel.GetSize().Y;
      this.dragAndBar.Location = vector2_1;
      this.dragAndBar.Location += this.dragAndBar.GetSize() * 0.5f;
      Vector2 _VSCale = vector2_1 + this.dragAndBar.GetSize();
      ...
      this.dragAndBar.Location += vector2_2;
      ZGenericText dragBarLabel = this.dragBarLabel;
      dragBarLabel.vLocation = dragBarLabel.vLocation + vector2_2;
