...
namespace TinyZoo.Z_BuildingInfo.Z_Processing.NewPanel.ProcessingValueView
{
  internal class ProcessingValueFrame
  {
      ...
      float ForcedHeight = -1f)
    {
      ...
      Vector2 vector2_1 = zero + this.view.GetSize();
      vector2_1.Y += num;
      this.customerFrame.Resize(vector2_1);
      this.miniHeading.SetTextPosition(vector2_1);
      Vector2 vector2_2 = -this.customerFrame.VSCale * 0.5f;
      this.catSelect.location += vector2_2;
      this.view.location.Y += vector2_2.Y;
      this.hackyScrollFrame_Top = new RowSegmentRectangle(vector2_1.X, num, ColourData.Z_FrameMidBrown, 1f);
      ...
      this.hackyScrollFrame_Bottom = new RowSegmentRectangle(vector2_1.X, num, ColourData.Z_FrameMidBrown, 1f);
