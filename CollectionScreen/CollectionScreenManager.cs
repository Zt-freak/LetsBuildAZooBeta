...
namespace TinyZoo.CollectionScreen
{
  internal class CollectionScreenManager
  {
      ...
      for (int index = 0; index < animalTypeList.Count; ++index)
      {
        if (!this.IsPetSelect || player.Stats.GetAliensCaptured(animalTypeList[index]) > 0)
        {
          if (this.IsCRISPR_Selector)
            _alienEntries.Add(new AlienEntry(animalTypeList[index], player.Stats.GetTotalVaiantsFound(animalTypeList[index]) == 10, player.Stats.GetTotalVaiantsFound(animalTypeList[index]) > 0, SCALEs: num));
          else
    ...
    {
      UIScaleHelper uiScaleHelper = new UIScaleHelper(SCALE);
      this.alienentries = _alienEntries;
      int num1 = 0;
      int num2 = 0;
      this.MaxThisRow = _MaxThisRow;
      this.Gap_X = this.alienentries[0].GetWidth() + uiScaleHelper.ScaleX(5f);
      this.Gap_Y = this.alienentries[0].GetHeight() + uiScaleHelper.ScaleY(5f);
      int num3 = 0;
