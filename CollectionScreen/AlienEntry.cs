...
namespace TinyZoo.CollectionScreen
{
  internal class AlienEntry : GameObject
  {
    ...
    {
      if (!this.UseNewRendering || this.animalInFrame == null)
        return;
    ...
    public float GetHeight(bool GetAnimalFrameOnly = false) => this.extraFrame != null && !GetAnimalFrameOnly ? this.extraFrame.VSCale.Y : this.scaleHelper.ScaleY((float) this.baseframe.DrawRect.Height);

    public float GetWidth(bool GetAnimalFrameOnly = false) => this.extraFrame != null && !GetAnimalFrameOnly ? this.extraFrame.VSCale.X : this.scaleHelper.ScaleX((float) this.baseframe.DrawRect.Width);
