...
namespace TinyZoo.Z_PenInfo.MainBar
{
  internal class PenInfoButton : GameObject
  {
    ...
    private float PageX;
    public int PAGE;
    private CustomerFrameMousOverBox mouseovertext;
      ...
      float _BaseScale = -1f)
    {
      ...
      if (this.tiletype != TILETYPE.Count)
        this.mouseovertext = new CustomerFrameMousOverBox(Z_GameFlags.GetBaseScaleForUI(), TileData.GetTileStats(this.tiletype).Name, 125f);
      ...
      switch (_ManageButtonType)
      {
        ...
        case BuildingManageButton.BuildPen_Enrichment:
          this.shortcuthighlight = new ShortcutHighlightIcon(OffscreenPointerType.NoEnrichment, this.BaseScale);
          break;
    ...
    public void Disable(bool BlockMouseClicks = true)
    {
      ...
      this.mouseovertext = new CustomerFrameMousOverBox(Z_GameFlags.GetBaseScaleForUI(), textToWrite, 80f);
      ...
      Player player)
    {
      ...
      if (this.tilecategory != TileCategory.None && !this.Disabled)
      {
        ...
        else if (this.tilecategory == TileCategory.Toilet)
        {
          ...
          flag = true;
        else if (this.ManageButtonType == BuildingManageButton.GetParkStaff && FeatureFlags.FlashHasApplicantsAtGate)
          flag = true;
