...
namespace TinyZoo.GenericUI
{
  internal class StringInBox : GameObject
  {
    ...
    public void UpdateStringInBox(float DeltaTime) => this.UpdateColours(DeltaTime);
      ...
      if (this.Frame != null)
      {
        ...
        if (this.LockedPixelScale && (this.UseRoundaboutFont || this.UseOnePointFiveFont))
        {
          if (this.IsAllCapsRoundAbout)
          {
            ...
            this.Frame.DrawGameObjectNineSlice(DrawWithThis, this.spritesheet, Offset + this.vLocation + vector2, this.VScale * Sengine.ScreenRatioUpwardsMultiplier);
          }
          else
            this.Frame.DrawGameObjectNineSlice(DrawWithThis, this.spritesheet, Offset + this.vLocation + new Vector2(0.0f, -4f * this.BaseScale), num1 * this.VScale * Sengine.ScreenRatioUpwardsMultiplier);
        }
        else
          this.Frame.DrawGameObjectNineSlice(DrawWithThis, this.spritesheet, Offset + this.vLocation, num1 * this.VScale * Sengine.ScreenRatioUpwardsMultiplier);
      }
      else
